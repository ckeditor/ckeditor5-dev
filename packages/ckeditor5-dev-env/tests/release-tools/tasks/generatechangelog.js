	...
	describe( 'generateChangelog()', () => {
		...
		beforeEach( () => {
			...
			stubs = {
				...
				transformCommitForCKEditor5Package: sandbox.spy(),
				transformCommitForCKEditor5DevPackage: sandbox.spy(),
				versionUtils: {
			...
			mockery.registerMock( '../utils/transformcommitforckeditor5package', stubs.transformCommitForCKEditor5Package );
			mockery.registerMock( '../utils/transformcommitforckeditor5devpackage', stubs.transformCommitForCKEditor5DevPackage );
		...
		it( 'generates changelog for version provided by a user', () => {
			...
			return generateChangelog()
				.then( () => {
					...
					expect( stubs.getWriterOptions.firstCall.args[ 0 ] ).to.equal( stubs.transformCommitForCKEditor5Package );
		...
		it( 'uses dev version of "transformCommit" function for dev package', () => {
			const newChangelogChunk = [
				'## 0.1.0',
				'',
				'### Features',
			...
			return generateChangelog( null, { isDevPackage: true } )
				.then( () => {
					const newChangelog = changelogUtils.changelogHeader + newChangelogChunk + '\n';
					...
					expect( stubs.getNewReleaseType.calledOnce ).to.equal( true );
					expect( stubs.getNewReleaseType.firstCall.args[ 0 ] ).to.equal( stubs.transformCommitForCKEditor5DevPackage );

					expect( stubs.getWriterOptions.calledOnce ).to.equal( true );
					expect( stubs.getWriterOptions.firstCall.args[ 0 ] ).to.equal( stubs.transformCommitForCKEditor5DevPackage );
		...
		it( 'committed changelog for dev package should not trigger CI', () => {
			const newChangelogChunk = [
				'## 1.0.0',
				'',
				'### Features',
			...
			return generateChangelog( '1.0.0', { isDevPackage: true } )
				.then( () => {
					expect( stubs.tools.shExec.secondCall.args[ 0 ] ).to.equal( 'git commit -m "Docs: Changelog for "test-package". [skip ci]"' );
				} );
		} );
	} );
} );
