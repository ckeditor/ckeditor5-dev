...
const MODULE_PATH = '../../../../lib/release-tools/utils/transform-commit/transformcommitforsubpackagefactory';

describe( 'dev-env/release-tools/utils/transform-commit', () => {
	describe( 'transformCommitForSubPackageFactory()', () => {
		let transformCommitForSubPackageFactory, sandbox, stubs, context;
		...
		beforeEach( () => {
			...
			stubs = {
				...
				transformCommitForSubRepositoryFactory: sandbox.stub(),
			...
			mockery.registerMock( './transformcommitforsubrepositoryfactory', stubs.transformCommitForSubRepositoryFactory );
			mockery.registerMock( './getchangedfilesforcommit', stubs.getChangedFilesForCommit );

			transformCommitForSubPackageFactory = proxyquire( MODULE_PATH, {
			...
			stubs.transformCommitForSubRepositoryFactory.callsFake( () => {
				// Factory pattern. The module must call a function that returns another function.
				return commit => {
					return commit;
				};
			} );
		...
		it( 'returns a function', () => {
			expect( transformCommitForSubPackageFactory() ).to.be.a( 'function' );
		} );

		describe( 'transformCommitForSubPackage()', () => {
			let transformCommitForSubPackage;

			beforeEach( () => {
				transformCommitForSubPackage = transformCommitForSubPackageFactory();
			} );
			...
			it( 'rejects the commit if no files were changed', () => {
				...
				expect( stubs.transformCommitForSubRepositoryFactory.called ).to.equal( false );
			} );
			...
			it( 'returns a new instance of object instead od modifying passed one', () => {
				...
				expect( stubs.transformCommitForSubRepositoryFactory.calledOnce ).to.equal( true );
			...
			it( 'accepts the commit when it has changed files in proper and other packages', () => {
				...
				expect( stubs.transformCommitForSubRepositoryFactory.called ).to.equal( true );
			...
			it( 'passed options to "transformCommitForSubPackageFactory()"', () => {
				transformCommitForSubPackage = transformCommitForSubPackageFactory( {
				...
				expect( stubs.transformCommitForSubRepositoryFactory.called ).to.equal( true );
				expect( stubs.transformCommitForSubRepositoryFactory.firstCall.args[ 0 ] ).to.deep.equal( {
					returnInvalidCommit: true
				} );
			} );
		} );
