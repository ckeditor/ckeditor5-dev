 ...
 * @returns {String|null}
 */
export function getChangesForVersion( version, cwd = process.cwd() ) {
 ...
 * @returns {String|null}
 */
export function getChangelog( cwd = process.cwd() ) {
 ...
 * @param {String} [cwd=process.cwd()] Where to look for the changelog file.
 */
export function saveChangelog( content, cwd = process.cwd() ) {
	const changelogFile = path.join( cwd, CHANGELOG_FILE );
 ...
 * @param {String} [cwd=process.cwd()] Where to look for the changelog file.
 */
export function truncateChangelog( length, cwd = process.cwd() ) {
