...
import { getReleasedPackagesInfo } from '../../src/utils/getreleasedpackagesinfo';
import type { SectionsWithEntries, PackageJson, SectionName, Entry } from '../../src/types';

const createEntry = (scope: string[], type: SectionName, breakingChange: SectionName, message: string = ''): Entry => ({
	message,
	data: {
		scope,
		type,
...
describe('getReleasedPackagesInfo', () => {
	...
	it('should return only other releases when no categorized changes exist', async () => {
		const packages: PackageJson[] = [{ name: '@ckeditor/uncategorized', version: '1.0.0' }];
		const sections = createSectionsWithEntries();
	...
	it('should remove duplicate package names in scope', async () => {
		...
		const packages: PackageJson[] = [{ name: '@ckeditor/core', version: '1.0.0' }];
