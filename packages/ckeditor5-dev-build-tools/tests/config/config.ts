...
describe( 'getRolldownConfig()', () => {
	...
	it( '--external automatically adds packages that make up the "ckeditor5"', async () => {
	...
	it( '--external automatically adds packages that make up the "ckeditor5-premium-features"', async () => {
		await mockGetUserDependency(
	...
	it( '--external rewrites CKEditor paths to aggregate packages in browser builds', async () => {
		await mockGetUserDependency(
			'ckeditor5/package.json',
			() => ( {
				name: 'ckeditor5',
	...
	it( '--external doesn\'t fail when "ckeditor5-premium-features" is not installed', async () => {
		await mockGetUserDependency(
			'ckeditor5-premium-features/package.json',
			() => {
	...
	it( 'configures a single CSS plugin for npm and browser builds', async () => {
