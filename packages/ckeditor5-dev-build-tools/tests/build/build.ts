...
describe( 'build()', () => {
	...
	it( 'TypeScript declarations', async () => {
		const { output } = await build( {
			input: 'src/input.ts',
			tsconfig: 'tsconfig.json',
			declarations: true
	...
	it( 'TypeScript source is preferred when a `.js` import has matching `.ts` and `.js` files', async () => {
		const { output } = await build( {
	 ...
	 * Browser
	 */
	it( 'Browser parameter set to `false`', async () => {
		const { output } = await build( {
			input: 'src/input.ts',
			tsconfig: 'tsconfig.json',
			browser: false
	...
	it( 'Browser parameter set to `true` and name parameter not set', async () => {
		const { output } = await build( {
			input: 'src/input.ts',
			tsconfig: 'tsconfig.json',
			browser: true
	...
	it( 'Browser parameter set to `true` and name parameter set', async () => {
		const { output } = await build( {
			input: 'src/input.ts',
			tsconfig: 'tsconfig.json',
			browser: true,
	...
	it.each( [ 'ckeditor5', 'ckeditor5-premium-features' ] )( 'Uses the JavaScript basename for %s CSS outputs', async outputName => {
		const { output } = await build( {
			input: 'src/input.ts',
	...
	it( 'Source map for chunk re-exporting external modules', async () => {
		const { output } = await build( {
	...
	it( 'Minification doesn\'t remove banner', async () => {
		const { output } = await build( {
			input: 'src/input.js',
			banner: 'src/banner.js',
			minify: true
	...
	it( 'Pure annotations preserve the marker used by post-processing tools', async () => {
		const { output } = await build( {
	 ...
	 * Error handling
	 */
	it( 'Throws error with nicely formatter message when build fails', async () => {
		setRolldownMock( () => ( {
			write() {
	...
	it( 'Throws error with file context using nicely formatter message when build fails', async () => {
		setRolldownMock( () => ( {
			write() {
	...
	it( 'Error with file context includes frame if provided', async () => {
		setRolldownMock( () => ( {
			write() {
				const err = new Error() as any;
	...
	it( 'Rewrites package imports to aggregate packages in browser builds', async () => {
	...
	it( 'should not throw when processing a file including a dynamic import expression', async () => {
		await expect( build( {
