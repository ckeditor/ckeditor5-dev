...
describe( 'bundleCss() (mocked)', () => {
	...
	it( 'Orders CSS modules by the JavaScript import order of the entry chunk', async () => {
	...
	it( 'Collects CSS imports of modules reachable only through an inlined dynamic import', async () => {
		bundleAsyncMock.mockImplementationOnce( async options => {
			const virtualEntry = options.resolver.read( '/__cke5_bundle_css__.css' );
			const specifiers = [ ...virtualEntry.matchAll( /"([^"]+)"/g ) ].map( match => match[ 1 ] );
	...
	it( 'Lets the facade module decide the CSS order regardless of the chunk module order', async () => {
		bundleAsyncMock.mockImplementationOnce( async options => {
			const virtualEntry = options.resolver.read( '/__cke5_bundle_css__.css' );
			const specifiers = [ ...virtualEntry.matchAll( /"([^"]+)"/g ) ].map( match => match[ 1 ] );
	...
	it( 'Traverses every module of entry and dynamic entry chunks without a facade module and caches shared results', async () => {
		bundleAsyncMock.mockImplementationOnce( async options => {
			const virtualEntry = options.resolver.read( '/__cke5_bundle_css__.css' );
			const specifiers = [ ...virtualEntry.matchAll( /"([^"]+)"/g ) ].map( match => match[ 1 ] );
	...
	it( 'Throws when Rolldown resolves a CSS import as external', async () => {
		bundleAsyncMock.mockImplementationOnce( async options => {
	...
	it( 'Uses the current working directory as the project root when output location is not specified', async () => {
		bundleAsyncMock.mockImplementationOnce( async options => {
	...
	it( 'Throws when a CSS import cannot be resolved by Rolldown', async () => {
		bundleAsyncMock.mockImplementationOnce( async options => {
	...
	it( 'Emits warnings for virtual entry diagnostics without a warning type', async () => {
