...
describe( 'bundleCss()', () => {
	it( 'bundles explicit editor and content roots into three self-contained assets in source order', async () => {
	...
	it( 'bundles monorepo source dependency roots into self-contained assets', async () => {
	...
	it( 'keeps the last occurrence of CSS entry points imported multiple times', async () => {
		const output = await generateBundle( {
			fileName: 'aggregate.css'
	...
	it( 'always emits all three assets when no CSS roots are imported', async () => {
	...
	it( 'throws when CSS is imported directly instead of through a theme entry point', async () => {
	...
	it( 'lists every CSS file imported outside a theme entry point in the error message', async () => {
	...
	it( 'allows minifying all generated bundles', async () => {
	...
	it( 'works with output.dir and preserved modules', async () => {
	...
	it( 'ignores raw CSS imports handled by the rawImport plugin', async () => {
	...
	it( 'leaves raw CSS imports to other plugins regardless of the plugin order', async () => {
		const bundle = await rolldown( {
	...
	it( 'derives editor and content bundle names from a fileName without the ".css" extension', async () => {
	...
	it( 'resolves package CSS imports via Rolldown resolution and ignores imports in comments and strings', async () => {
	...
	it( 'emits Lightning CSS warnings through Rolldown warnings', async () => {
	...
	it( 'throws when encountering external CSS imports', async () => {
	...
	it( 'throws when encountering protocol-relative external CSS imports', async () => {
		await expect( generateBundle( { fileName: 'styles.css' }, './fixtures/input-external-import-protocol-relative.ts' ) )
			.rejects.toThrow( 'External CSS imports are not supported' );
	} );

	it( 'registers the CSS entry points and their nested imports as watch files', async () => {
